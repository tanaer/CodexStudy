...
![playwright-mcp-add-command](../images/playwright-mcp-add-command.png)

**验证安装是否成功：** 进入 Codex，使用 `/mcp` 命令列出当前已经安装的 MCP 服务列表。

![playwright-mcp-server-list](../images/playwright-mcp-server-list.png)

![playwright-mcp-list-command](../images/playwright-mcp-list-command.png)
...
![playwright-mcp-browser-search-prompt](../images/playwright-mcp-browser-search-prompt.png)
...
![playwright-mcp-permission-prompt](../images/playwright-mcp-permission-prompt.png)

在执行过程中，你确实会发现它打开了浏览器，并且搜索了相关内容，还打开了两篇文章，这些都是可以看到的。

![playwright-mcp-baidu-search-results](../images/playwright-mcp-baidu-search-results.png)

最终它会把得到的结果总结输出给我们，然后**写入到本地的 Markdown 文件**里。

![playwright-mcp-collected-search-summary](../images/playwright-mcp-collected-search-summary.png)

Codex 根据搜索到的两篇文章的内容进行总结，给我们进行相关的阐述说明。

![playwright-mcp-markdown-file-created](../images/playwright-mcp-markdown-file-created.png)
